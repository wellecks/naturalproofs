...
class Classifier(pl.LightningModule):
    def __init__(self, hidden_dim=1024, num_layers=2, lr=1e-4, vocab_size=0, dropout=0.0, log_every=50, pad_idx=0):
        super().__init__()
    ...
    def _fwd_packed(self, inp, lstm):
        lens = inp.ne(self.hparams.pad_idx).sum(1).cpu()
        emb = self.emb(inp)
    ...
    def encode_x(self, x):
        return self._fwd_packed(x, self.x_encoder)

    def encode_r(self, r):
        return self._fwd_packed(r, self.r_encoder)
